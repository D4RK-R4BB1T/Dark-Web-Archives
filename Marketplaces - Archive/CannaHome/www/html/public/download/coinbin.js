$(document).ready(function() {
	...
	$("#signBtn").click(function(){
		...
		var isMultipleTransactions = false;
		...
		// sort out incorrectly formatted transactions
		script.val(cleanUpTransactions(script.val()));
		
		// detect multiple transactions, Delimited by one or more line-breaks
		if((script.val()).match(REGEX_MULTIPLE_TRANSACTIONS)){
			$(script).parent().removeClass('has-error');
			var isMultipleTransactions = true;
		} else if((script.val()).match(/^[a-f0-9]+$/ig)){
			$(script).parent().removeClass('has-error');
		} else {
		...
		if($("#sign .has-error").length==0){
			...
			if(isMultipleTransactions){
				$('#signBtn').attr('value', 'Signing...');
				signMultipleTransactions(
					{
						unsigned: parseMultipleTransactions(script.val()),
						signed: []
					},
					wifkey.val()
				);
	...
	/* verify script code */
	/* batch verification and signing */

	const REGEX_MULTIPLE_TRANSACTIONS = /^[a-f0-9]*(?:\n+[a-f0-9]+)+$/ig;

	// Remove unnecessary tabs
	$('.nav.navbar-nav > li:first-child').remove();
	$('a[href="#broadcast"]').parent().remove();
	$('a[href="#wallet"]').parent().remove();
	$('a[href="#about"]').parent().remove();
	
	// Add disclaimer
	$('#footer .text-muted').append(' <span class="text-primary">(modified to allow verification and signing of multiple transactions at once)</span>');
	$('#about').append('<h3 class="text-danger">This is a modified version of coinb.in</h3><p>This version of coinbin has been modified to allow verification and signing of multiple transactions at once. Additionally, certain functionality has been disabled.</p>');
	
	// Add coin switcher
	$(".nav.navbar-nav").append('<li><select class="form-control" id="coinSwitcher" style="margin-top: 7px;"><option selected="">Bitcoin</option><option>Litecoin</option></select></li>');
	$("#coinSwitcher").on(
		'change',
		function(){
			switch ($(this).children(":selected")[0].innerText){
				case 'Bitcoin':
					coinjs.pub = 0x00;
					coinjs.priv = 0x80;
	...
	$('#verifyBtn').after(' <a href="#sign" class="btn btn-success hidden" id="multiSignBtn">Sign Transactions <span class="glyphicon glyphicon-chevron-right"></span></a>');

	$('#multiSignBtn').on(
		'click',
		function(){
			$("#signTransaction").val( $("#verifyScript").val()  );
			$("#signPrivateKey").focus();
		}
	);

	function parseMultipleTransactions(script){
		return script.replace(/\n+/g, ',').split(',');
	}
	
	function cleanUpTransactions(script){
		return script.replace(/[^a-f0-9\n]/g,'');
	}
	
	function decodeMultipleTransactionScripts(){
		// sort out incorrectly formatted transactions
		var verifyScript = cleanUpTransactions( $("#verifyScript").val() );
		
		var tx = coinjs.transaction();
		
		if( verifyScript.match(REGEX_MULTIPLE_TRANSACTIONS) ){
			var transactions = parseMultipleTransactions(verifyScript);
			
			$("#verifyTransactionData .transactionRBF").hide();
			$("#verifyTransactionData").removeClass("hidden");
			$("#verifyTransactionData tbody").html("");
			$("#multiSignBtn").removeClass('hidden');
			
			try{
				var addresses = [];
				
				$.each(transactions, function(i, t){
					var decode = tx.deserialize(t);
					...
					$.each(decode.outs, function(i,o){
						if(o.script.chunks.length==2 && o.script.chunks[0]==106){ // OP_RETURN
							...
							var isMainOutput = false;
							if(i == 0){
								isMainOutput = true;
								biggestOutputAddress = addr;
							}
																				
							h += '<tr' + (isMainOutput ? ' class="info"' : false) + '>';
							h += '<td><input class="form-control" type="text" value="'+addr+'" readonly></td>';
							h += '<td class="col-xs-1">'+(o.value/100000000).toFixed(8)+'</td>';
							h += '<td class="col-xs-2"><input class="form-control" type="text" value="'+Crypto.util.bytesToHex(o.script.buffer)+'" readonly></td>';
							h += '</tr>';							
					...
					addresses.push(biggestOutputAddress);
					
					$(h).appendTo("#verifyTransactionData .outs tbody");
				});
				
				$('#verifyTransactionData').append('<div id="verifyElectrum"><label>Verify Ownership of Output Addresses (Electrum Wallet)</label><p>Use the console command below to verify that the output addresses of this transaction belong to your Electrum wallet.</p><p>Copy the command-code below, paste it into the prompt in the <em>console</em> tab of your Electrum wallet, and press <kbd>enter</kbd>.</p><p>If all the addresses belong to the wallet, Electrum will output <strong>true</strong>.</p><p>For transactions with multiple outputs, only the <strong class="text-info">main output</strong> (the one of highest value) is verified.</p><table class="table table-striped table-hover outs"><thead><tr style="font-weight:bold;"><td></td><td><abbr data-original-title="the amount the address is being sent" title="">Electrum Console Verification Command</abbr></td></tr></thead><tbody><tr><td class="col-xs-3">Verifies ownership of main output addresses in each transaction</td><td><textarea rows="2" class="form-control" readonly="">all(map(ismine, ' + JSON.stringify(addresses) + '))</textarea></td></tr></tbody></table></div>');
	...
	function signMultipleTransactions(
		transactions,
		wifkey
	){
		setTimeout(
			function(){
				if(transactions.unsigned.length == 0){
					var signedOutput = transactions.signed.join(',');
					$("#signedData textarea").val(signedOutput);
					$("#signedData").removeClass('hidden').fadeIn();
					$('#signBtn').attr('value', 'Submit');
				} else {
					unsignedTransaction = transactions.unsigned.pop();
					
					var tx = coinjs.transaction();
					var t = tx.deserialize(unsignedTransaction);

					var signedTransaction = t.sign(wifkey);
					transactions.signed.push(signedTransaction);
					
					if( unsignedTransaction.length == signedTransaction.length ){
						$("#signedDataError").removeClass('hidden');
						$("#signedData").addClass('hidden');
						$('#signBtn').attr('value', 'Submit');
					...
					signMultipleTransactions(transactions, wifkey);
				}
			},
			1000
		);
	...
	$("#verifyBtn").click(function(){
		$(".verifyData, #multiSignBtn").addClass("hidden");
		$("#verifyStatus").hide();
		$('#verifyElectrum').remove();
		if(!decodeMultipleTransactionScripts()){
