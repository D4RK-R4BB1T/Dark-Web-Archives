...
class RawTransaction
{
    ...
    public static function decode($raw_transaction, $address_version = '00')
    {
        ...
        $info['locktime'] = hexdec(self::_return_bytes($raw_transaction, 4, TRUE));
        return $info;
    }
     ...
     * @return    boolean
     */
    public static function validate_signed_transaction($raw_tx, $json_string, $address_version = '00', &$signatures = 0)
    {
     ...
     * @return    string/FALSE
     */
    public static function create($inputs, $outputs, $magic_byte = '00', $lock_Time = 0)
    {
        ...
        foreach ($inputs as $input) {
            ...
            $tx_array['vin'][] = array('txid' => $input['txid'],
                'vout' => $input['vout'],
                'sequence' =>
					isset($input['sequence'])
						?	$input['sequence']
						:	isset($lock_Time)
							? 4294967294
							: 4294967295,
                'scriptSig' => array('hex' => (isset($input['scriptSig']) ? $input['scriptSig'] : ''))
            );
        }
