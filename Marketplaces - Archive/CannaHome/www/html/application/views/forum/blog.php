<?php
	$URLPrefix = URL . 'blog/' . $this->blog['Alias'] . '/';
	
	//$this->blog
	
	$numberOfPages = ceil($this->blog['PostCount']/BLOG_POSTS_PER_PAGE);
?>
<div class="rows-30">
	<h2 class="row band">
		<span><?php echo $this->blog['Title']; ?></span>
		<div>
		<?php
			...
			<a class="btn <?php echo $this->blog['Subscribed'] ? 'red' : 'blue'; ?>" href="<?php echo URL . 'forum/toggle_subscription/blog/' . $this->blog['ID'] . '/' . $this->sortMode . '/' . $this->pageNumber . '/'; ?>">
				<i class="<?php echo Icon::getClass('RSS'); ?>"></i>
				<?php echo $this->blog['Subscribed'] ? 'Unsubscribe' : 'Subscribe'; ?>
			</a>
			<?php 
	...
	<ul class="row list-posts">
		<?php
			$i = 0;
			foreach($this->blog['BlogPosts'] as $blogPost) { 
				$firstComment =
					$i == 0 &&
					$this->pageNumber == 1;
				$veryLastComment =
					$this->pageNumber == $numberOfPages &&
					$i == count($this->blog['blogPosts']) - 1;
	...
	<?php if( $this->blog['PostCount'] > BLOG_POSTS_PER_PAGE ){ ?>
	<div class="row panel">
		<?php
