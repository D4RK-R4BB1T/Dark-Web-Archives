...
Description: Custom place edit page
--}}
@extends('layouts.master')

@section('title', 'Редактирование кастомного места')
...
@section('content')
    ...
    [
        ...
        ['title' => 'Редактирование кастомного места']
    ]])
    ...
    <div class="row">
        ...
        <div class="col-sm-12 col-lg-13 animated fadeIn">
            <div class="well block">
                <h3>Редактирование кастомного места: {{ $good->title }}</h3>
                <hr class="small" />
                <form action="" role="form" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row">
                                ...
                                <div class="form-group condensed has-feedback {{ $errors->has('region') ? 'has-error' : '' }}">
                                    <select name="region" class="form-control" title="Округ">
                                        ...
                                        @foreach ($city->regions as $region)
                                            <option value="{{ $region->id }}" {{ (old('region') ?: $place->region_id) == $region->id ? 'selected' : '' }}>{{ $region->title }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="col-xs-{{ in_array($city->id, \App\City::citiesWithRegions()) ? '16' : '24' }}">
                            <div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
                                <input id="title" type="text" class="form-control" name="title" placeholder="Название места" value="{{ old('title') ?: $place->title }}" required>
                    ...
                    <div class="text-center">
                        <button type="submit" class="btn btn-orange">Отредактировать место</button>
                    </div>
                </form>
            </div>
        </div> <!-- /.col-sm-12 -->
