...
@section('title', 'Создание группы :: Скидки')
        ...
        <div class="col-sm-12 col-lg-13 animated fadeIn">
            <div class="well block">
                ...
                <div class="alert alert-warning">
                    После создания группы новые пользователи не будут перенесены туда автоматически. <br />
                ...
                <form action="" role="form" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-xs-24">
                            <div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
                                <input id="title" type="text" class="form-control" name="title" placeholder="Название группы (не отображается пользователю)" value="{{ old('title') }}" required {{ autofocus_on_desktop() }}>
                    ...
                    <div class="row">
                        <div class="col-xs-24">
                            <div class="form-group{{ $errors->has('percent_amount') ? ' has-error' : '' }}">
                                <input id="percent_amount" type="text" class="form-control" name="percent_amount" placeholder="Величина скидки (%), только цифры" value="{{ old('percent_amount') }}" required>
                        ...
                        <div class="col-xs-24">
                            <div class="form-group has-feedback{{ $errors->has('mode') ? ' has-error' : '' }}">
                                <select name="mode" class="form-control" title="Режим перехода">
                                    ...
                                    <option value="{{ \App\UserGroup::MODE_AUTO }}" {{ old('mode') == \App\UserGroup::MODE_AUTO ? 'selected' : '' }}>Переход по количеству покупок</option>
                                    <option value="{{ \App\UserGroup::MODE_MANUAL }}" {{ old('mode') == \App\UserGroup::MODE_MANUAL ? 'selected' : '' }}>Ручной режим</option>
                                </select>
                    ...
                    <div class="row">
                        <div class="col-xs-24">
                            <div class="form-group{{ $errors->has('buy_count') ? ' has-error' : '' }}">
                                <input id="buy_count" type="number" class="form-control" name="buy_count" placeholder="Количество покупок для авто-перехода" value="{{ old('buy_count') }}">
                    ...
                    <div class="text-center">
                        <button type="submit" class="btn btn-orange">Создать группу</button>
                    </div>
                </form>
            </div>
        </div> <!-- /.col-sm-12 -->
