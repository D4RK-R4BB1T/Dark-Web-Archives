    ...
    <div class="panel-heading">
        ...
        <span class="dropdown pull-right">
            ...
            <ul class="dropdown-menu dropdown-menu-right" role="menu">
                @if(Auth::user()->isSecurityService() || Auth::user()->isModerator())
                <li role="presentation"><a class="dark-link" role="menuitem" tabindex="-1" href="{{ url("/shop/service/messages/new") }}"><i class="glyphicon glyphicon-envelope"></i> Отправить сообщение</a></li>
                @endif
                <li role="presentation"><a class="dark-link" role="menuitem" tabindex="-1" href="{{ url("/messages/delete") }}"><i class="glyphicon glyphicon-trash"></i> Удалить выбранные диалоги</a></li>
            </ul>
        </span>
    </div>
                ...
                @foreach($threads as $thread)
                    <?php $latestMessage = $thread->getLatestMessage(); ?>
                        @if(!isset($deleting) || !$deleting)
                            <a href="{{ url('/messages/'.$thread->id.'?page='.request('page', 1)) }}" class="dark-link">
                        @endif
                        <div class="list-group-item {{ $thread->isUnread(Auth::id()) ? 'message-unread' : '' }}" style="display: table; table-layout: fixed; width: 100%; padding: 10px 6px">
                            ...
                            <div class="col-xs-20 col-sm-18 col-md-19 col-lg-20">
                                <p style="margin-bottom: 0; word-wrap: break-word; text-overflow: ellipsis; overflow: hidden; white-space: nowrap">
                                    ...
                                    @if (Auth::user()->employee)
                                        <small>Участники:</small> <small class="text-muted">{!! $thread->participantsString(Auth::id()) ?: '-' !!}</small>
                                    @endif
                                </p>
                                <p class="semi-small" style="margin-bottom: 0; word-wrap: break-word; text-overflow: ellipsis; overflow: hidden; white-space: nowrap">
